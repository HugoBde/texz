...
pub fn move_cursor_down() !void {
    buffer.cursor.row = @min(buffer.cursor.row + 1, display.display_dimensions.ws_row);
}
pub fn move_cursor_up() !void {
    buffer.cursor.row = @max(buffer.cursor.row - 1, 1);
}
pub fn move_cursor_right() !void {
    buffer.cursor.col = @min(buffer.cursor.col + 1, display.display_dimensions.ws_col);
}
